...
const Section = forwardRef<HTMLDivElement, SectionProps>((props, ref) => {
  const {variant = 'primary', ReadMoreButton = true, isTitle = false, isOpinion = false, backgroundColor = ''} = props;
  return (
    <section className={styles.content} style={{backgroundColor: backgroundColor, padding: backgroundColor !== '' ? '2rem' : ''}}>
      {props.title && <div className={styles.titleContent} data-after={`${props.title === 'Opinión' ? 'false' : isOpinion}`}>
        {!ReadMoreButton && <hr className={styles.startLine}
          style={{backgroundColor: variant === 'primary' ? 'var(--section-title-color)' : 'var(--common-white)'}}
        />}
        {props.titleUrl ? (
            ...
            <>
              {
                isTitle ? <h1 className={styles.title}
                  style={{color: variant === 'primary' ? 'var(--section-title-color)' : 'var(--common-white)'}}>
                  {props.title}
                </h1> :
        ...
        {!ReadMoreButton ? <hr className={styles.line}
          style={{backgroundColor: variant === 'primary' ? 'var(--section-title-color)' : 'var(--common-white)'}}
        /> :
          <a className={styles.readMore} style={{color: variant === 'primary' ? 'var(--section-title-color)' : 'var(--common-white)'}} href={props.titleUrl ?? ''}></a>}
      </div>}
      {ReadMoreButton && <hr className={styles.line}
        style={{backgroundColor: variant === 'primary' ? 'var(--section-title-color)' : 'var(--common-white)'}}
      />}
